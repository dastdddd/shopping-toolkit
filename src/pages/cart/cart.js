...
const Cart = () => {
  ...
  const total = cart.reduce((acc, item) => acc + (Math.floor(item.price - item.price * item.discountPercentage / 100)), 0)
  ...
  const postOrderProduct = async (form) => {
    const date = new Date();
    let a = moment(date).locale('ky').format('DD/MM/YY HH:mm:ss');
    try {
  ...
  return (
    <>
      <div style={{
        ...
        <button onClick={() => dispatch(openToggleModal(true))}>Оформить заказ</button>
        ...
        <Box sx={{
        ...
        }}>
          {
            cart && cart.map(item => <ListItem
              //некоторые приходят из пропса потому что {...item} приходят все пропсы
              color='#d63031'
              onClick={() => dispatch(deleteItem(item.id))}
              btnText='Удалить из карзину'
              display='block'
              key={item.id}
              {...item}
            />)
