...
export default function ListItem({brand,id, description,title,price, thumbnail,rating, discountPercentage, onClick, btnText, color, display}) {
      ...
      <CardActionArea>
        ...
        <CardContent>
          ...
          <Typography 
            ...
            color="text.secondary"
            sx={{fontSize: '20px'}}>
            Сумма скидка: {Math.floor(discount)}$
          </Typography>
        </CardContent>
      </CardActionArea>
      <CardActions>
        ...
        //Пропстарды ушул жерге жаздык чтобы list.js те башка ат менен жана cart.js те башка ат менен 
          ...
          >
          {btnText}
          {display === 'null' ? <ShoppingCartOutlinedIcon/> : <DeleteOutlineOutlinedIcon/>}
